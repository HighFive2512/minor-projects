...
def init_db():
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS Holidays (id INTEGER PRIMARY KEY, holiday_date DATE UNIQUE, holiday_name TEXT)')
    cursor.execute('CREATE TABLE IF NOT EXISTS DaysOff (id INTEGER PRIMARY KEY, member_id INTEGER, day_off_date DATE, reason TEXT, FOREIGN KEY (member_id) REFERENCES TeamMembers(id))')
    conn.commit()
    conn.close()
